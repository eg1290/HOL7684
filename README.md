...
## HOL7684 Session Title: Building Node.js-MongoDB Container Packaged Applications Using Wercker ##
...
On the dashboard click the menu on the top left corner. The list of services will fly in from the left. Select **Container Classic**. to open Oracle Container Cloud Services console.
![](images/04.navigate.occs.png)

Now you have to see an already provisioned Container Cloud Service named *oow*. Click the hamburger icon on the left and select **Container Console** from the dropdown menu.

![alt text](images/30.occs.open.admin.console.png)

Due to the reason that the certification hasn't been setup you will get a security warning. Ignore that and allow to open the page. Enter the Administrator's credential for your Container Cloud Service. Username: *admin* and password: same for Oracle Public Cloud Service account distributed by instructor. Click **Login**.
